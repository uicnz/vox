...
private let appLogger = VoxLog.app
private let cacheLogger = VoxLog.caches

class VoxAppDelegate: NSObject, NSApplicationDelegate {
	...
	func applicationDidFinishLaunching(_: Notification) {
		...
		// Ensure FluidAudio caches live under Application Support, not ~/.cache
		...
		Task {
			...
			await soundEffect.setEnabled(voxSettings.soundEffectsEnabled)
		}
	...
	private var shouldOpenForegroundUIOnLaunch: Bool {
		// When Vox launches at login, stay quietly in the menu bar regardless of
	...
	private func startLifecycleTasksIfNeeded() {
		Task { @MainActor in
			await VoxApp.appStore.send(.task).finish()
		}
	}
    ...
    private func configureLocalCaches() {
        do {
            let cache = try URL.voxApplicationSupport.appendingPathComponent("cache", isDirectory: true)
	...
	func presentMainView() {
		...
		let transcriptionStore = VoxApp.appStore.scope(state: \.transcription, action: \.transcription)
	...
	func presentSettingsView() {
		...
		let settingsView = AppView(store: VoxApp.appStore)
	...
	private func updateAppMode() {
		appLogger.debug("showDockIcon = \(self.voxSettings.showDockIcon)")
		if self.voxSettings.showDockIcon {
