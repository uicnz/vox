...
struct AppFeature {
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case .transcription(.modelMissing):
        VoxLog.app.notice("Model missing - activating app and switching to settings")
        state.activeTab = .settings
        state.settings.shouldFlashModelSection = true
        return .run { send in
          await MainActor.run {
            VoxLog.app.notice("Activating app for model missing")
  ...
  private func startPasteLastTranscriptMonitoring() -> Effect<Action> {
    .run { send in
      ...
      let handler = PasteLastTranscriptHotkeyHandler()

      let token = keyEventMonitor.handleKeyEvent { keyEvent in
        let isSetting = isSettingPasteLastTranscriptHotkeyRef.withLock { $0 }
        let pasteHotkey = voxSettingsRef.withLock { $0.pasteLastTranscriptHotkey }
        return handler.handle(
          keyEvent,
          isSettingPasteLastTranscriptHotkey: isSetting,
  ...
  private func ensureSelectedModelReadiness() -> Effect<Action> {
    .run { send in
      @Shared(.voxSettings) var voxSettings: VoxSettings
      @Shared(.modelBootstrapState) var modelBootstrapState: ModelBootstrapState
      let selectedModel = voxSettings.selectedModel
...
// The key event monitor stores synchronous @Sendable callbacks. This handler
// snapshots shared settings under lock before deciding whether to intercept the
// paste-last-transcript hotkey.
private final class PasteLastTranscriptHotkeyHandler: @unchecked Sendable {
  private let lock = NSLock()

  func handle(
    _ keyEvent: KeyEvent,
    isSettingPasteLastTranscriptHotkey: Bool,
    pasteHotkey: HotKey?,
    send: Send<AppFeature.Action>
  ) -> Bool {
    lock.lock()
    defer { lock.unlock() }
